...
cwd <- "/Users/bruessow/NPI/latest8-poor-pirate/traces/traces-pirat/"
...
pos9data15 <- subset(data15,position==9)
data15$trial <- as.factor(data15$trial)


means15 <- with(pos9data15, tapply(RT,IND=list(condition),mean))
stderr15 <- with(pos9data15,tapply(RT,IND=list(condition),se))
tn15 <- with(pos9data15,tapply(RT,IND=list(condition),length))
tu15 <- with(pos9data15,means15 + qt(.975,df=tn15-1) * stderr15)
tl15 <- with(pos9data15,means15 - qt(.975,df=tn15-1) * stderr15)
...
pos9datapre <- subset(datapre,position==9)
datapre$trial <- as.factor(datapre$trial)


meanspre <- with(pos9datapre, tapply(RT,IND=list(condition),mean))
stderrpre <- with(pos9datapre,tapply(RT,IND=list(condition),se))
tnpre <- with(pos9datapre,tapply(RT,IND=list(condition),length))
tupre <- with(pos9datapre,meanspre + qt(.975,df=tnpre-1) * stderrpre)
tlpre <- with(pos9datapre,meanspre - qt(.975,df=tnpre-1) * stderrpre)
...
datameansTRTpos9 <- c(322.0851, 338.9567, 388.6175, 306.0881, 279.2141, 275.4075) 
datatuTRTpos9 <- c(352.9892, 375.5709, 425.5560, 337.1555, 306.5616, 300.6031) 
datatlTRTpos9 <- c(291.1811, 302.3425, 351.6789, 275.0207, 251.8667, 250.2119) 
...
#createPS("preNPI100runs/Mar189PM.ps")
...
### chunk number 12: 
###################################################
#createPS("twomodels/Mar2010AM.ps")
...
### chunk number 14: 
###################################################
#createPS("preNPI/Mar20.ps")
#createPS("preNPI/Mar21.ps")

cols <-  gray(0:1 / 2)

#modeldatameanspos9 <- rbind(meanspre,datameansTRTpos9)
#modeldatatupos9 <- rbind(tupre,datatuTRTpos9)
#modeldatatlpos9 <- rbind(tlpre,datatlTRTpos9)
...
### chunk number 16: 
###################################################
#createPS("NPI500runs/Mar1920060727hrs.ps")
#createPS("NPI100runs/Mar2120060600hrs.ps")
#createPS("NPI/Mar212006.ps")
...
props <- read.table("/Users/shravanvasishth/Desktop/SvenlatestNPI/Mar212006/ans030-100runs/ans030props.txt")
%props <- read.table("/Users/bruessow/NPI/latest8-poor-pirate/traces/traces-pirat/Mar212006/ans030-100runs/ans030props.txt")
...
c(a.prop.correct,b.prop.correct,c.prop.correct,d.prop.correct,e.prop.correct,f.prop.correct)*100
